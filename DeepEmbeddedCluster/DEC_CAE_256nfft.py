def CAE_for_DEC(n1, n2, n3, mindim, k): # starting for gsi file
    ...
    x = Conv2D(nfilt*4, (2, 1), activation='relu', strides=(2,1))(x)
    x = Conv2D(nfilt*8, (2, 1), activation='relu', strides=(1,1))(x) 
 
    x = Conv2D(nfilt*8, (2, 1), activation='selu', strides=(2,1))(x) 
    x = Flatten()(x)
    encoded = Dense(mindim, activation='relu',name='encoded')(x)
    x = Dense(nfilt*8*5*4, activation='relu')(encoded)
    x = Reshape((5,4,nfilt*8))(x)
    
    x = Conv2DTranspose(nfilt*4, (2, 1), activation='relu', strides=(2,1))(x)
    
    x = Conv2DTranspose(nfilt*4, (2, 1), activation='relu', strides=(1,1))(x)
    x = Conv2DTranspose(nfilt*2, (2, 1), activation='relu',  strides=(2,1))(x)
    
    x = Conv2DTranspose(nfilt*2, (1, 2), activation='relu', strides=(1,1))(x)
